...
#ifndef USE_NAMED_IDLE_PROCS
/*	USE_NAMED_IDLE_PROCS controls whether or not we use the macro that
	replaces the HFT proc for AVAppRegisterProc.  This macro will call
	the AVAppRegisterIdleProcWithName proc that passes in both the 
	source file name and the name of the proc.
	By defining it as 0/1 here, modules must opt-in/opt-out by
	defining themselves.	*/
#define USE_NAMED_IDLE_PROCS 0
#endif // USE_NAMED_IDLE_PROCS

#ifndef USE_NAMED_LATE_INIT_PROCS
/*	USE_NAMED_LATE_INIT_PROCS controls whether or not we use the macro that
	replaces the HFT proc for AVAppRegisterLateInitProc.  This macro will call
	the AVAppRegisterLateInitProccWithName proc that passes in both the 
	source file name and the name of the proc.
	By defining it as 0/1 here, modules must opt-in/opt-out by
	defining themselves.	*/
...
/**
	...
	@param bundle The icon bundle that is used to create resized icon.
	@param width Width of new icon.
	@param height Height of new icon.
	@return A valid icon if resize is successful.
	@see AVAppCreateIconBundle6
 */
typedef ACCBPROTO1 AVIcon (ACCBPROTO2 *AVIconHandlerResizeProc)(const AVIconBundle6 bundle, ASInt32 width, ASInt32 height);
/** 
	@ingroup AVIcon
*/
...
/**
	...
	@see AVAnnotHandlerPostDrawProc
...
enum
{
	...
	/** Opens the AVDoc in a new window. If not specified, the AVDoc will open in a new tab by default. */
	kAVDocOpenInNewWindow	= 0x00000001
};
typedef ASUns32 AVDocOpenFlags;
...
@param annotHandler IN The annotation handler responsible
for this annotation.
@param anAnnot IN The annotation.
@param pageView IN The AVPageView in which the annotation
is located.
@param updateRect IN The portion of the annotation's bounding
rectangle that should be drawn.
@see AVAnnotHandlerDrawProc
*/
typedef ACCBPROTO1 void (ACCBPROTO2 *AVAnnotHandlerPostDrawProc)(
	AVAnnotHandler annotHandler,
	PDAnnot anAnnot,
	AVPageView pageView,
	AVDevRect *updateRect);
...
enum {
	...
	/** DEPRECATED: **always Set as TRUE** We have removed CloudConnectors from Open/Save Dialog. 
		To reduce change, we set this value in AVAppMac.cpp and wvapp.cpp */
	kAVDoNotAllowCloudConnections				= 1 << 6,  

	/** Allow Acrobat.com Entry points **/
	/** DEPRECATED ** - This flag is deprecated and nothing happens on using it. We will remove it soon. */
...
typedef struct {								
	...
	*/
    ...
    /* Visibility of export checkbox depends on showExportCheckbox.*/
	ASBool										  showExportCheckbox;
} AVOpenSaveDialogParamsRec, *AVOpenSaveDialogParams;
    
#endif

/**
    A structure defining the output parameters related to a file save
    dialog box. It is used for AVAppSaveDialogEx().
    @see AVAppSaveDialogEx
*/
typedef struct {
    ...
    /** The index of selected AVFileFilterRec object in <code>fileFilters</code>. */
    ASInt16     chosenFilterIndex;
        
    /** Exported file is launched only if launchFile is set to true. */
    ASBool      launchFile;
}AVOpenSaveDialogOutParamsRec, *AVOpenSaveDialogOutParams;
...
/* AVVersionParamsRef */
typedef struct {
	ASSize_t size;				// Size of the struct
	ASUns32 major;				// Major version number
	ASUns32 minor;				// Minor version number
	ASUns32 minor_minor;		// Minor_minor version number
	ASUns32 minor_minor_minor;	// Build version number
} AVVersionParamsRec, *AVVersionParams;

/**
A newer version of AVExtensionInfo. This supports the new version format.
Use pVersionParams to access version related information. For everything
else, use the pExtensionInfo member which points to the same content as
obtained using the older AVExtensionAcquireInfo API.
@see AVExtensionAcquireInfoEx
@see AVExtensionReleaseInfoEx
*/
typedef struct _AVExtensionInfoRecEx {
	/** The size of the structure. It must be set to <code>sizeof(AVExtensionInfoRecEx)</code>. */
	ASSize_t 			size;

	/* Pointer to AVExtensionInfoRec containing basic info*/
	AVExtensionInfo pExtensionInfo;

	/* Pointer to AVVersionParamsRec containing version info*/
	AVVersionParams pVersionParams;
} AVExtensionInfoRecEx, *AVExtensionInfoEx;

#ifdef __cplusplus
}

// Common periods for idle procs.  NOTE: procs that stay registered for a long period of time 
// should always be registered at a period of 1 second or more.  
...
#define AVAppRegisterIdleProc(proc, data, period) AVAppRegisterIdleProcWithName(proc, data, period, __FILE__, #proc)
#endif // USE_NAMED_IDLE_PROCS

#if USE_NAMED_LATE_INIT_PROCS
#define AVAppRegisterLateInitProc(proc, data, period) AVAppRegisterLateInitProcWithName(proc, data, period, __FILE__, #proc)
#endif // USE_NAMED_LATE_INIT_PROCS

#if USE_NAMED_IDLE_PROCS
#define AVAppRegisterIdleProcEx(proc, data, lowPriority, initialDelay, repeating, period, maxPeriod) AVAppRegisterIdleProcExWithName(proc, data, lowPriority, initialDelay, repeating, period, maxPeriod, __FILE__, #proc)
#define AVAppRegisterLowPriorityIdleProc(proc, data, initialDelay, period) AVAppRegisterIdleProcExWithName(proc, data, true, initialDelay, true, period, kPeriodInfinite, __FILE__, #proc)
#define AVAppRegisterLowPriorityDelayedProc(proc, data, initialDelay) AVAppRegisterIdleProcExWithName(proc, data, true, initialDelay, false, kPeriodInfinite, kPeriodInfinite, __FILE__, #proc)
#else
#define AVAppRegisterIdleProcEx(proc, data, lowPriority, initialDelay, repeating, period, maxPeriod) AVAppRegisterIdleProcExWithName(proc, data, lowPriority, initialDelay, repeating, period, maxPeriod, "", "")
#define AVAppRegisterLowPriorityIdleProc(proc, data, initialDelay, period) AVAppRegisterIdleProcExWithName(proc, data, true, initialDelay, true, period, kPeriodInfinite, "", "")
#define AVAppRegisterLowPriorityDelayedProc(proc, data, initialDelay) AVAppRegisterIdleProcExWithName(proc, data, true, initialDelay, false, kPeriodInfinite, kPeriodInfinite, "", "")
#endif
