...
/**
	...
	Note : There are two versions here. 
	NEVER call the AVAppRegisterIdleProcOLD(...) proc. It is for binary
	compatability only. DON'T EVER CALL IT.
	...
	@since PI_ACROVIEW_VERSION >= 0x00020000
*/
#if USE_NAMED_IDLE_PROCS
NPROC(void, AVAppRegisterIdleProcOLD, (AVIdleProc idleProc, void *clientData, ASUns32 period))
#else
NPROC(void, AVAppRegisterIdleProc, (AVIdleProc idleProc, void *clientData, ASUns32 period))
#endif // USE_NAMED_IDLE_PROCS
...
/**
	...
	<TR><TD><code>UICustomizationContextMenu</code></TD><TD>The context menu for SIM menu items.</TD></TR>
	</TABLE>
...
@since PI_ACROVIEW_VERSION >= 0x00070000
*/
#if USE_NAMED_LATE_INIT_PROCS
NPROC(void, AVAppRegisterLateInitProcOLD, (AVIdleProc lateInitProc, void *clientData, ASUns32 delay))
#else
NPROC(void, AVAppRegisterLateInitProc, (AVIdleProc lateInitProc, void *clientData, ASUns32 delay))
#endif // USE_NAMED_LATE_INIT_PROCS
...
/**
	...
	by the AVAppRegisterIdleProc MACRO that is defined in AVExp.T.
	That Macro supplies the pPrgFileName and the pProcName parameters.
	...
	but it may be called less frequently. <code>period</code> is specified 
	in ticks (one tick is 1/60 of a second)
	@param pPgmFileName is a cstring of the file name that calls AVAppRegisterIdleProc
	@param pProcName is a cstring of the procedure name
	@see AVAppUnregisterIdleProc 
	@see AVAppModalWindowIsOpen
	@since PI_ACROVIEW_VERSION >= 0x000B0006
*/
NPROC(void, AVAppRegisterIdleProcWithName, (AVIdleProc idleProc, void *clientData, ASUns32 period, const char* pPgmFileName, const char* pProcName))

NPROC(void, AVAppRegisterLateInitProcWithName, (AVIdleProc idleProc, void *clientData, ASUns32 period, const char* pPgmFileName, const char* pProcName))

/**
	Unregisters a user-supplied procedure to call after a context 
	menu has been created but before it is shown to the user. 
	Used to unregister a call created with AVAppRegisterForContextMenuAddition.
	The arguements must exactly match the ones used in the register call, or
	this unregister will fail.
	@param menuName The name of the context menu to modify. Its names can be one of the following:
	...
	to the procedure each time it is called. 
	@exception genErrNoMemory 
	@since PI_ACROVIEW_VERSION >= 0x000C0000
*/
NPROC(void, AVAppUnregisterForContextMenuAddition, (ASAtom menuName, AVContextMenuAdditionProc proc, void *clientData))

/*
 Displays a platform-dependent file save dialog box with View Result check box.
 
 @ingroup ReplaceableMethods
 @param dialogParams IN Specifies standard dialog params for Save dialogs.
 @param outDialogParams OUT holds output parameters for Save dialogs.
 @return <code>true</code> if settings were valid and output export params are obtained successfully. 
 <code>false</code> if the user clicked Cancel or some error occurred.
 @since PI_ACROVIEW_VERSION >= 0x000C0000
 */
PROC(ASBool, AVAppSaveDialogEx, (AVOpenSaveDialogParams dialogParams,
                                  AVOpenSaveDialogOutParams outDialogParams))
...
/*
 ...
 @since PI_ACROVIEW_VERSION >= 0x000C0000
*/
NPROC(ASBool, AVAppBeginSaveEx, (AVOpenSaveDialogParams	dialogParams,
								AVOpenSaveDialogOutParams outDialogParams,
								ASPathName *outASPathNameToWrite,
								AVAppFileSaveHandle		*outFileSaveHandle))
...
/**
	...
	This is an superset of both AVAppRegisterIdleProc and AVAppRegisterDelayedProc.
	By setting parameters, the behavior of both AVAppRegisterIdleProc and 
	AVAppRegisterDelayedProc can be achieved as well as	having an idle proc 
	that starts with a delay and a lower priority proc that doesn't interrupt critical
	processing.

	In general, you source code should never call this.  It is called
	by the AVAppRegisterIdleProcEx MACRO that is defined in AVExpT.h.
	That Macro supplies the pPrgFileName and the pProcName parameters.
	AVAppRegisterLowPriorityIdleProc(proc, data, initialDelay, period) and
	AVAppRegisterLowPriorityDelayedProc(proc, data, initialDelay) 
	are supplied for convenience and are also defined in AVExpT.h.

	The following are common periods for idle procs.  NOTE: procs that stay registered
	for a long period of time should always be registered at a period of 1 second or more.

	#define kInitalDelayImmediate 0
	#define kPeriodImmediate 1
	#define kPeriodQuick 5
	#define kPeriodQuarterSecond 15
	#define kPeriodHalfSecond 30
	#define kPeriodOneSecond 60
	#define kPeriodMediumEnergyImpact 300
	#define kPeriodLowEnergyImpact 600
	#define kPeriodInfinite ASMAXUns32
	...
	but it may be called less frequently. <code>period</code> is specified 
	in ticks (one tick is 1/60 of a second)
	@param lowPriority a low priority proc can be delayed by page rendering, clicks etc.
	this should be set to low priority if it is not critical to run right away
	@param initialDelay The time in ticks before this <code>idleProc</code> should be 
	called the first time.  For non-repeating idle procs, this is the value that determains
	the delay an period is not used.
	@param repeating specify if this <code>idleProc</code> should be run once and 
	unregistered or if it should be continually called based on the period.  If this is false,
	the values for period and maxPeriod are not used.  Consider using AVAppRegisterDelayedProc if
	this is not repeating and isn't low priority
	@param maxPeriod specifies the maximum amount of time in ticks that the 
	<code>idleProc</code> is able to wait before being needing to be called.
	This can be set so that a low priority proc is not starved it has been continually 
	deferred from running.
	@param pPgmFileName is a cstring of the file name that calls AVAppRegisterIdleProc
	@param pProcName is a cstring of the procedure name
	@see AVAppUnregisterIdleProc 
	@see AVAppModalWindowIsOpen
	@since PI_ACROVIEW_VERSION >= 0x000C0000
*/
NPROC(void, AVAppRegisterIdleProcExWithName, (AVIdleProc idleProc, void *clientData, ASBool lowPriority, ASUns32 initialDelay, ASBool repeating, ASUns32 period, ASUns32 maxPeriod, const char* pPgmFileName, const char* pProcName))


/**
Returns the version number of the product using the <code>AVVersionParams</code> structure.  Replaces the AVAppGetVersion API, which was limited to due to sizing.

@param avVersionParams OUT A user-supplied pointer to an AVVersionParamsRec.
@see AVAppGetVersion
@since PI_ACROVIEW_VERSION >= 0x000C0000
*/
NPROC(void, AVAppGetVersionEx, (AVVersionParams avVersionParams))

NOPROC(AVStorageRegisterNewStorage_deprecated)
NOPROC(AVStorageRegisterAccount_deprecated)
NOPROC(AVStorageGetAccountInfo_deprecated)
NOPROC(AVPerformStorageOAuth2Authentication_deprecated)
NOPROC(AVMakeWebServicesRequest_deprecated)

/**
Fills an AVExtensionInfoRecEx structure with some information
about a client. It extends the AVExtensionInfoRec API which does not support
new versioning scheme. Similar to AVExtensionAcquireInfo, it is the caller's 
responsibility to release the memory associated with the contents of 
the returned structure by calling AVExtensionReleaseInfoEx().

@param nIndex The index of the client for which to retrieve information.
The minimum value for <code>index</code> is zero; the maximum is
the return value of <code>AVExtensionGetNumPlugIns() - 1</code>.
@return The structure containing information about the client.
@see AVExtensionReleaseInfoEx

@note This API is not supported on UNIX platforms.
@since PI_ACROVIEW_VERSION >= 0x000C0003
*/
NPROC(AVExtensionInfoEx, AVExtensionAcquireInfoEx, (AVArraySize nIndex))

/**
Releases the memory associated with the contents of <code>info</code>.
@param info The AVExtensionInfoRecEx structure to release.
@see AVExtensionAcquireInfoEx

@note This API is not supported on UNIX platforms.
@since PI_ACROVIEW_VERSION >= 0x000C0003
*/
NPROC(void, AVExtensionReleaseInfoEx, (AVExtensionInfoEx info))

/**
 * Brings the specified document to front. i.e. Makes its window and main view visible.
 * @param doc The AVDoc to bring to front.
 * @return TRUE if doc was successfully brought to front. FALSE otherwise.
 * @since PI_ACROVIEW_VERSION >= 0x000C0003
 */
NPROC(ASBool, AVDocBringToFront, (AVDoc doc))
/**
 * Gets the display-title of a given doc.
 * Display title is the text that appears on application title bar when the doc is the active document.
 * @param[in] doc AVDoc object for which display-title is desired. It is not necessary for doc to be active.
 * @param[out] title ASText object that will receive the display title.
 * @return <code>true</code> If title is assigned a value, <code>false</code> otherwise.
 * @note
 * # Display-title only contains portion of title that appears as window title before the name of application.
 * # If the same AVDoc is opened in two different windows or tabs (try 'Window->New Window' menu-item)
 *    the current implementation returns display-text as if AVDoc is opened in only one window.
 * # Parameter title remains unmodified if false is returned.
 * # Always returns <code>false</code> if doc or title is NULL.
 * @since PI_ACROVIEW_VERSION >= 0x000C0003
 */
NPROC(ASBool, AVDocGetDisplayTitle, (AVDoc doc, ASText title))
/**
 * Creates dynamically scalable icon from PDF.
 * @param pdDoc PDDoc object from which icon should be created.
 * @param pageNum Page number of PDF which should be used to create icon.
 * @param width Default width of icon.
 * @param height Default height of icon.
 * @note Returned AVIcon is scalable and icons with different size are created on need basis.
 *			width and height parameters are used as default icon size when needed.
 *			For example when returned value is used as icon of AVToolButton.
 * It is safe and recommended to close the PDDoc, using PDDocClose(), just after creating icons from PDF.
 * @since PI_ACROVIEW_VERSION >= 0x000C0003
 */
NPROC(AVIcon, AVIconCreateFromPDF, (PDDoc pdDoc, ASInt32 pageNum, ASInt32 width, ASInt32 height))
/**
 * Assigns icon to tool created by using AVToolBarNew().
 * @param toolbar Toolbar for which icon needs to be assigned.
 * @param normalIcon Icon that should be used for the tool in tool center.
 * @param activeIcon Icon that should be used when tool is active, e.g. in tool switcher which appears when tool is active.
 * @since PI_ACROVIEW_VERSION >= 0x000C0003
 * @note
 * # toolbar must have been created by calling AVToolBarNew().
 * # It is expected that normalIcon and activeIcon both should be scalable icons.
 * @see AVIconCreateFromPDF.
 */
 NPROC(ASBool, AVToolBarSetIcon, (AVToolBar toolbar, AVIcon normalIcon, AVIcon activeIcon))
